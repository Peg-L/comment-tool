...
export class PanelUI {
  ...
  _buildPanel() {
    ...
    toggle.appendChild(
      this._icon(this._collapsed ? "mdi:chevron-right" : "mdi:chevron-left"),
    );
    ...
    this._pickBtn = this._btn(
      "mdi:cursor-default-click",
      "選取元素",
      "btn-primary",
      () => this._emit("pickRequest"),
    );
    this._visBtn = this._btn(
      "mdi:eye-off-outline",
      "隱藏標註",
      "btn-neutral",
      () => this._emit("toggleOverlay"),
    );
    const exportBtn = this._btn(
      "mdi:content-copy",
      "複製 Prompt",
      "btn-success",
      () => this._emit("exportPrompt"),
    );
    const copyDataBtn = this._btn(
      "mdi:database-export-outline",
      "複製資料",
      "btn-teal",
      () => this._emit("exportData"),
    );
    const importBtn = this._btn(
    ...
    this._projFlyoutBtn.appendChild(
      this._icon("mdi:file-document-multiple-outline"),
    );
    const projNameSpan = this._doc.createElement("span");
    projNameSpan.className = "proj-name";
    projNameSpan.style.cssText =
  ...
  _rerenderLast() {
    ...
    this.refresh(
      this._lastRefresh.comments,
      this._lastRefresh.isMissing,
      this._lastRefresh.onLocate,
    );
  }
  ...
  _toggleCollapse() {
    ...
    if (toggle) {
      ...
      if (icon)
        icon.setAttribute(
          "icon",
          this._collapsed ? "mdi:chevron-right" : "mdi:chevron-left",
        );
    }
  }
  ...
  _openFlyout() {
    ...
    this._doc.addEventListener("click", this._flyoutOutsideHandler, {
      capture: true,
    });
  }
  ...
  _closeFlyout() {
    ...
    if (this._flyoutOutsideHandler) {
      this._doc.removeEventListener("click", this._flyoutOutsideHandler, {
        capture: true,
      });
      this._flyoutOutsideHandler = null;
    }
  ...
  setNoProject(val) {
    ...
    if (this._listEl && this._noProject) {
      this._listEl.innerHTML =
        '<div class="empty">頁面清單初始化中，請稍後再開始標註。</div>';
    }
  }
